...
#   curl -fsSL https://raw.githubusercontent.com/joomidang-tech/heysenlyt_pi/v1.2.0/install.sh \
#     | sudo bash -s -- https://senlyt.com                 # prod
#     | sudo bash -s -- https://dev-env.senlyt.com         # dev
#     | sudo bash -s -- https://v1-2-0.env.senlyt.com      # 버전 프리뷰
#
# pi 코드는 이 파일이 놓인 브랜치(v1.2.0 — 위 URL 의 버전)에서 받는다. 환경 구분은 맨 끝 서버 URL 하나로만.
...
BRANCH="${SENLYT_INSTALL_BRANCH:-v1.2.0}"
