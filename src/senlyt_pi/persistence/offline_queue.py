...
(2026-09-04 헥사고날 감사 P1로 pipeline→persistence 이관 — 이 모듈은 내구 버퍼/스풀이지
오케스트레이션이 아니어서, adapters 가 참조해도 방향(core·persistence ← adapters)이 성립한다.
구 경로 `pipeline.offlinequeue` 는 호환 shim 으로 유지.)
